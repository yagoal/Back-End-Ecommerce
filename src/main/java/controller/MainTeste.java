...
public class MainTeste {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		Usuarios que constam no método popularBancoDeDados(), senha: testejava
		...
		Funcionario funcionarioTeste = funcionarioDao.buscarPorId(usuario);
		...
		try {
			if (opt != 0) {
				...
				if (clienteTeste.autenticacao(usuario, senha)) {
					...
					System.out.println("Cliente: " + clienteTeste.getNome() + " logado com sucesso");
					
					int contPagamento = 0; //controlador apenas para não seguir cadastrando quando for pagar outros pedidos
					int contCancelamento = 0; //controlador apenas para não seguir cadastrando quando for cancelar outros pedidos
						...
						while (true) {
							...
							if (optUsuario == 1) {
								...
								*/
								...
								Produto produto2 = produtoDao.buscarPorId(10l);
								Produto produto3 = produtoDao.buscarPorId(11l);
								...
								List<Pedido> pedidosPendentes = pedidoDao.buscarPedidosPorStatus(StatusPedido.PENDENTE);
								...
								if (pedidosPendentes.isEmpty()) {
									...
									if (optPagamentoConfirm == 1) {
										...
										pedidosPendentes = pedidoDao.buscarPedidosPorStatus(StatusPedido.PENDENTE);
										if (!pedidosPendentes.isEmpty()) {
											pedidoDao.imprimirPedidos(pedidosPendentes);
										} else {
								...
								*/
								...
								pedido2.setStatus(StatusPedido.PAGO, StatusPagamento.PAGO);
								...
								List<Pedido> pedidosPendentes = pedidoDao.buscarPedidosPorStatus(StatusPedido.PENDENTE);
								List<Pedido> pedidosPagos = pedidoDao.buscarPedidosPorStatus(StatusPedido.PAGO);
								...
								if (pedidosPendentes.isEmpty() && pedidosPagos.isEmpty()) {
									...
									if (optCancelamentoConfirm == 1) {
										...
										pedidosPendentes = pedidoDao.buscarPedidosPorStatus(StatusPedido.PENDENTE);
										pedidosPagos = pedidoDao.buscarPedidosPorStatus(StatusPedido.PAGO);
				...
				} else { //else menu funcionário
					
				}
					if (funcionarioTeste.autenticacao(usuario, senha)) {
						
						System.out.println("Funcionario: " + funcionarioTeste.getNome() + " logado com sucesso");
						
						
						while (true) {
							System.out.println("Digite a Opcao Referente ao que deseja fazer");
							System.out.println("1: Para gerenciar pedidos");
							System.out.println("2: Para gerenciar produtos");
							System.out.println("Para sair digite qualquer outros");

							int optUsuario = leitor.nextInt();
							
							if(optUsuario == 1) {
								
								System.out.println("Digite usuario do cliente que deseja gerenciar o pedido");
								String usuarioBusca = leitor.next();
								Cliente cliente = clienteDao.buscarPorId(usuarioBusca);
								
								Produto produto1 = produtoDao.buscarPorId(8l);
								Produto produto2 = produtoDao.buscarPorId(3l);
								Produto produto3 = produtoDao.buscarPorId(6l);
								...
								List<Pedido> listPago = pedidoDao.buscarPedidosPorStatus(StatusPedido.PAGO);
								List <Pedido> listPendente = pedidoDao.buscarPedidosPorStatus(StatusPedido.PENDENTE);

								System.out.println("Digite a Opcao Referente ao que deseja fazer");
								System.out.println("1: Para enviar pedidos");
								System.out.println("2: Para cancelar pedido");
								
								int optPedido = leitor.nextInt();
								
								if (optPedido == 1) {
									
									pedidoDao.imprimirPedidos(listPago);
									
									System.out.println("Digite id do pedido que deseja enviar");
									Long optGerenciar = leitor.nextLong();
									Pedido pedidoGerenciar = pedidoDao.buscarPorId(optGerenciar);
									
									pedidoGerenciar.enviarPedido();
									
									em.getTransaction().begin();
									pedidoDao.atualizar(pedidoGerenciar);
									em.getTransaction().commit();
									
								} else if (optPedido == 2) {
									pedidoDao.imprimirPedidos(listPago);
									pedidoDao.imprimirPedidos(listPendente);
									
									System.out.println("Digite id do pedido que deseja cancelar");
									Long optGerenciar = leitor.nextLong();
									Pedido pedidoGerenciar = pedidoDao.buscarPorId(optGerenciar);
									
									pedidoGerenciar.cancelarPedido();
								...
								int optProduto = leitor.nextInt();
								produtoDao.imprimirCatalogo();
								
								if (optProduto == 1) {
									
									System.out.println("Qual id do Produto que você deseja atualizar");
									
									long idProduto = leitor.nextInt();
									Produto produtoGerenciar = produtoDao.buscarPorId(idProduto);
									
									System.out.println("O que você deseja atualizar do produto");
									System.out.println("1: Para adicionar ou remover a quantidade de um produto");
									System.out.println("2: Para alterar a descrição de um produto");
									System.out.println("3: Para alterar o preço de um produto");
									System.out.println("Qualquer outro para sair");
									
									int optGerenciar = leitor.nextInt();
									
									if (optGerenciar == 1) {
										System.out.println("Quanto você deseja acrescentar ou remover");
										int setAlteracao = leitor.nextInt();
										int qtdAtual = produtoGerenciar.getQtd();
										System.out.println("1 Para adicionar ou outro para remover");
										
										int optAddRem = leitor.nextInt();
										if (optAddRem == 1) {
											produtoGerenciar.setQtd( qtdAtual + setAlteracao);
										} else {
											produtoGerenciar.setQtd( qtdAtual - setAlteracao);
											if (produtoGerenciar.getQtd() < 0) {
												System.out.println("Não é possível ter número de produtos negativo");
												produtoGerenciar.setQtd(qtdAtual);
											}
										}
										
									} else if (optGerenciar == 2) {
										System.out.println("Digite uma nova descrição para o o produto " + produtoGerenciar.getNome());
										String setDesc = leitor.next();
										
										produtoGerenciar.setDescricao(setDesc);
										
										
										
									} else if (optGerenciar == 3) {
										System.out.println("Digite um novo valor para o o produto " + produtoGerenciar.getNome());
										
										BigDecimal setPreco = leitor.nextBigDecimal();
										
										produtoGerenciar.setPreco(setPreco);
									...
									em.getTransaction().begin();
									produtoDao.atualizar(produtoGerenciar);
									em.getTransaction().commit();
								} else if (optProduto == 2) {
									produtoDao.imprimirCatalogo();
									System.out.println("Qual id do Produto que você deseja remover");
									
									long idProduto = leitor.nextInt();
									Produto produtoGerenciar = produtoDao.buscarPorId(idProduto);
									
									em.getTransaction().begin();
									produtoDao.remover(produtoGerenciar);
									em.getTransaction().commit();
								}
								produtoDao.imprimirCatalogo();
	...
	private static void popularBancoDeDados() {
		...
		Endereco endereco = new Endereco("48607235", "Avenida da Amizade", "1069", "General Dutra", "Paulo Afonso",
				"Bahia");
		Endereco endereco1 = new Endereco("48608490", "Rua da Harmonia", "S/N", "Alves de Souza", "Paulo Afonso",
				"Bahia");
		Endereco endereco2 = new Endereco("48605605", "Avenida Centenário", "650", "Centenário", "Paulo Afonso",
				"Bahia");
		...
		Endereco endereco5 = new Endereco("48000600", "Avenida Getulio Vargas", "48601-000", "Centro", "Paulo Afonso",
				"Bahia");
		...
		Funcionario funcionario = new Funcionario("fabianovaz", "testejava", "Fabiano Vaz", "56250944036", 1);
		Funcionario funcionario1 = new Funcionario("eliomar1", "testejava", "Eliomar Campos", "34029207006", 2);
		Funcionario funcionario2 = new Funcionario("julymota", "testejava", "Julyana Mota", "93161976002", 3);
		...
		funcionarioDao.cadastrar(funcionario2);
